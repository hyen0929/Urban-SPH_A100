					...
					if(g_str[k]!=cu_memset){
						...
						for(int_t j=g_str[k];j<fend;j++){
							...
							if(tdist<search_range){
								...
								// twij=calc_kernel_wij(tmp_Ai,hi,tdist);
								...
								// if((p_type_j!=-1)){  // KDH
								// 	tmpx+=uxj*twij*(p_type_j>0);
								// 	tmpy+=uyj*twij*(p_type_j>0);
								// 	flt+=twij*(p_type_j>0);
								// }
							}
						}
...
__global__ void KERNEL_Neumann_boundary2D(int_t*g_str,int_t*g_end,part1*P1,part2*P2,part3*P3)
{
	...
	Real search_range, hi, tmp_A;
	Real tflt_s;
	Real tpres, ttemp, thpres, tux, tuy, aix, aiy, aiz;
	hi=P1[i].h;
	tmp_A=calc_tmpA(hi);
	// tmp_A=calc_tmpA(hi);
	search_range=k_search_kappa*hi;	// search range
	...
	tux=tuy=ttemp=tpres=thpres=tflt_s=0.0;
	...
	for(int_t y=-P1[i].ncell;y<=P1[i].ncell;y++){
		for(int_t x=-P1[i].ncell;x<=P1[i].ncell;x++){
			...
			if(g_str[k]!=cu_memset){
				int_t fend=g_end[k];
				for(int_t j=g_str[k];j<fend;j++){
					Real xj,yj,tdist,uxj,uyj,mj,rhoj,presj,tempj;
					...
					if(P1[j].p_type<1000){
						...
						if(tdist<search_range){
							...
							// thpres+=((P3[j].ftotalx)*(xj-xi)+(P3[j].ftotaly)*(yj-yi))*rhoj*mj/rhoj*twij*(ptype_j>=1);
							thpres+=((P3[j].ftotalx)*(xj-xi)+(P3[j].ftotaly)*(yj-yi))*mj/rhoj*twij*(ptype_j>=1);
							ttemp+=tempj*twij*mj/rhoj*(ptype_j>=1);
...
__global__ void KERNEL_open_boundary_extrapolation2D(Real ttime, int_t*g_str,int_t*g_end,part1*P1,part2*P2,part3*P3,int_t tcount,Real tdt)
{
	...
	Real xi, xgnode_i, yi,ygnode_i;
	Real rho_ref_i;
	Real search_range, hi, tmp_A, trho, tdrho_x, tdrho_y, tux, tdux_x, tdux_y, tuy, tduy_x, tduy_y;
	Real drho_x, drho_y;
				...
				for(int_t j=g_str[k];j<fend;j++){
					...
					if(P1[j].p_type<1000){
						...
						if(tdist<search_range){
							...
							tux+=uxj*twij*volj*(ptype_j==1);
							tdux_x+=uxj*tdwx*volj*(ptype_j==1);
							tdux_y+=uxj*tdwy*volj*(ptype_j==1);

							tuy+=uyj*twij*volj*(ptype_j==1);
							tduy_x+=uyj*tdwx*volj*(ptype_j==1);
							tduy_y+=uyj*tdwy*volj*(ptype_j==1);

							trho+=rhoj*twij*volj*(ptype_j==1);
							tdrho_x+=rhoj*tdwx*volj*(ptype_j==1);
							tdrho_y+=rhoj*tdwy*volj*(ptype_j==1);

							tpres+=volj*presj*twij*(ptype_j==1);
							tdpres_x+=presj*tdwx*volj*(ptype_j==1);
							tdpres_y+=presj*tdwy*volj*(ptype_j==1);
	...
	if (k_open_boundary==1){
		if(P1[i].buffer_type==Outlet){
			...
			Real tB,tP,tmpres;
			if((P1[i].eli<0.9999)&&(P1[i].p_type>0))	P1[i].m=P1[i].rho*P1[i].vol;
		}
	}
}
////////////////////////////////////////////////////////////////////////
