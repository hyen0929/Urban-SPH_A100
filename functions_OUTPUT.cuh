...
void save_vtk_bin_single_fluid(part1*P1,part2*P2,part3*P3)
{
	int_t i,nop;//,nob;
	nop=num_part2;
	...
	for(i=0;i<nop;i++) if((P1[i].i_type==1)&&(P1[i].p_type==1)) {
		Nparticle++;
		plot_flag[i]=1;
	}
	...
	printf("Saving fluid plot to '%s', count=%d\n",DirName,count);
	...
	for (int ccount=0;ccount<num_plot_data;ccount++)
	{
		...
		if (!strncmp(data_label,"vorticity",5)) {
			fprintf(outFile_vtk,"vorticity\t3\t%d\tfloat\n",Nparticle);
			for(i=0;i<nop;i++){
				//if(P1[i].x>0){
				// if((P1[i].p_type==2)|(P1[i].p_type==9)){
				if(plot_flag[i]==1){
					val=FloatSwap(P3[i].omx);
					fwrite((void*)&val,sizeof(float),1,outFile_vtk);
					val=FloatSwap(P3[i].omy);
					fwrite((void*)&val,sizeof(float),1,outFile_vtk);
					val=FloatSwap(P3[i].omz);
					fwrite((void*)&val,sizeof(float),1,outFile_vtk);
				}
			}
		}
...
void save_vtk_bin_single_LDM(L_part1*LP1)
{
	...
	nop=num_part_LDM;
	// nob=number_of_boundaries;
	int_t Nparticle=0;							// number of LDM particles (x>0.00) for 3D PGSFR calculation
	int_t *plot_flag;
	plot_flag = (int*)malloc(sizeof(int)*nop);
	//for(i=0;i<nop;i++) if(P1[i].x>0) Nparticle++;
	//for(i=0;i<nop;i++) if(((P1[i].i_type==1)||(P1[i].i_type==2))&&P1[i].y>=0) {
	...
	printf("Number of LDM Particles = %d\n\n",Nparticle);
	...
	// Filename: It should be series of frame numbers(nameXXX.vtk) for the sake of auto-reading in PARAVIEW.
	char FileName_vtk[256];
	char DirName[256]="CaseHS_LDM(Frontfar)";
	sprintf(FileName_vtk,"./%s/LDM_%dstp.vtk",DirName, count);
	printf("Saving LDM plot to '%s', count=%d\n",DirName,count);
	// If the file already exists,its contents are discarded and create the new one.
	FILE*outFile_vtk;
	outFile_vtk=fopen(FileName_vtk,"w");
...
void save_vtk_bin_single_boundary(part1*P1,part2*P2,part3*P3)
{
	int_t i,nop;//,nob;
	nop=num_part2;
	// nob=number_of_boundaries;
	int_t Nparticle=0;							// number of boundary particles (x>0.00) for 3D PGSFR calculation
	int_t *plot_flag;
	plot_flag = (int*)malloc(sizeof(int)*nop);
	//for(i=0;i<nop;i++) if(P1[i].x>0) Nparticle++;
	//for(i=0;i<nop;i++) if(((P1[i].i_type==1)||(P1[i].i_type==2))&&P1[i].y>=0) {
	for(i=0;i<nop;i++) if((P1[i].i_type==1)&&(P1[i].p_type==0)) {
		Nparticle++;
		plot_flag[i]=1;
	}
	printf("Number of Boundary Particles = %d\n\n",nop);
	...
	char FileName_vtk[256];
	char DirName[256]="CaseHS_LDM";
	sprintf(FileName_vtk,"./%s/boundary_%dstp.vtk",DirName, count);
	printf("Saving boundary plot to '%s', count=%d\n",DirName,count);
	// If the file already exists,its contents are discarded and create the new one.
	FILE*outFile_vtk;
	outFile_vtk=fopen(FileName_vtk,"w");
...
void save_vtk_bin_single_buildings(part1*P1,part2*P2,part3*P3)
{
	int_t i,nop;//,nob;
	nop=num_part2;
	// nob=number_of_boundaries;
	int_t Nparticle=0;							// number of building particles (x>0.00) for 3D PGSFR calculation
	int_t *plot_flag;
	plot_flag = (int*)malloc(sizeof(int)*nop);
	//for(i=0;i<nop;i++) if(P1[i].x>0) Nparticle++;
	//for(i=0;i<nop;i++) if(((P1[i].i_type==1)||(P1[i].i_type==2))&&P1[i].y>=0) {
	for(i=0;i<nop;i++) if((P1[i].i_type==1)&&(P1[i].p_type==1000)) {
		Nparticle++;
		plot_flag[i]=1;
	}
	printf("Number of Building Particles = %d\n\n",nop);
	...
	char FileName_vtk[256];
	char DirName[256]="CaseHS_LDM";
	sprintf(FileName_vtk,"./%s/building_%dstp.vtk",DirName, count);
	printf("Saving building plot to '%s', count=%d\n",DirName,count);
	// If the file already exists,its contents are discarded and create the new one.
	FILE*outFile_vtk;
	outFile_vtk=fopen(FileName_vtk,"w");
...
void save_vtk_bin_single_vorticity(part1*P1,part2*P2,part3*P3)
{
	int_t i,nop;//,nob;
	nop=num_part2;
	// nob=number_of_boundaries;
	...
	//for(i=0;i<nop;i++) if(((P1[i].i_type==1)||(P1[i].i_type==2))&&P1[i].y>=0) {
	for(i=0;i<nop;i++) if((P1[i].i_type==1)&&(P1[i].p_type==1)) {
		Real omega = sqrt(P3[i].omx*P3[i].omx+P3[i].omy*P3[i].omy+P3[i].omz*P3[i].omz);
		if(omega>0.1) {
			Nparticle++;
			plot_flag[i]=1;
		}
	...
	char FileName_vtk[256];
	char DirName[256]="CaseHS_LDM";
	sprintf(FileName_vtk,"./%s/vorticity_%dstp.vtk",DirName, count);
	printf("Saving fluid plot to '%s', count=%d\n",DirName,count);
	// If the file already exists,its contents are discarded and create the new one.
	FILE*outFile_vtk;
	outFile_vtk=fopen(FileName_vtk,"w");
